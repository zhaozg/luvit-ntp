...
local function tointeger(bytes)
    local b = { bytes:byte(1, 4) }
    local hw = b[1] * 256 + b[2]
    local lw = b[3] * 256 + b[4]
    return hw * 65536 + lw
end

function NTPClient:calc_stamp(secbytes, msecbytes)
    local sec = tointeger(secbytes) + self.tz     -- NTP-stamp, seconds since 1.1.1900
    if not msecbytes then
        return sec - 1104494400 - 1104494400      -- UNIX-timestamp, seconds since 1.1.1970
    else
        sec = sec - 1104494400 - 1104494400
    end
    local msec = math.floor(tointeger(msecbytes) / 4294.967296)
    return sec, msec
end
...
function NTPClient:run(interval)
    assert(self.interval==nil and type(interval)=='number', "invalid interval")
